...
public class UserMobileController : UserController
{
    ...
    private void Start()
    {
        ...
        //DebugMobile.Log("Swipe pixel delta 1.0 is: " + maxPixelDelta);
    }
    ...
    private void Update()
    {
        ...
        //DebugMobile.PrintLabel1($"Double tap timer: {doubleTapTimer}");
        if (tapPress1 && tapRelease1)
            doubleTapTimer += Time.deltaTime;
        else
            doubleTapTimer = 0;

        //DebugMobile.PrintLabel2($"Tap hold timer: {tapHoldTimer}");
        if (tapPress1 && !tapRelease1)
            tapHoldTimer += Time.deltaTime;
        else
            tapHoldTimer = 0;

        // Отслеживаем ДВОЙНОЙ тап
        //if (doubleTapTimer <= doubleTapThreshold)
        //{
        //    if (tapPress1 && tapRelease1 && tapPress2 && tapRelease2)
        //        CatchDoubleTap();
        //}
        //else
        //{
        ...
        //DebugMobile.PrintLabel3($"H1={(tapPress1?'T':'F')} R1={(tapRelease1 ? 'T' : 'F')}" +
           // $" H2={(tapPress2 ? 'T' : 'F')} R2={(tapRelease2 ? 'T' : 'F')}");
    }
